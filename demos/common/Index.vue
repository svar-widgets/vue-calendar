	...
	<div class="top-layout" :class="{ narrow: isMobileView }">
		<div class="sidebar" :class="{ active: show }" role="tabpanel">
			<div class="sidebar-content">
				...
				<div class="box-links">
					<template v-for="group in links" :key="group.group">
						<div v-if="group.group" class="group-title">{{ group.group }}</div>
						<Link
...
.header-back-btn .btn-box :deep(button.toggle-btn i) {
	color: var(--demo-framework-color);
}
...
.top-layout :deep(div.segmented-themes svg) {
	height: 16px;
	width: 16px;
}
